...
/** Near-entrance treatment wing: short walks, full coverage of all 6 conditions. */
...
/** Same coverage moved to the far north edge: every treatment walk is ~30 tiles. */
const FAR_ROOMS: RoomSpec[] = STANDARD_ROOMS.map((r) => ({
  ...r,
  rect: { ...r.rect, row: r.rect.row - 24 },
  door: r.door ? { ...r.door, row: r.door.row - 24 } : null,
}));

const STANDARD_STAFF: { role: RoleId; count: number }[] = [
  { role: 'nurse', count: 2 },
  { role: 'doctor', count: 2 },
  { role: 'radTech', count: 1 },
...
describe('headless balance harness (M4)', () => {
  it('a sensible hospital survives 3 days in the black and treats the load', () => {
    const s = runHospital(1337, STANDARD_ROOMS, 3);
    const w = s.world;
