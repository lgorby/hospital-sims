...
- **Art pass** (tech plan §2.6 contract — `characterKey()`, the prop-slice lookups, and the `PROP_DECOR` table are atlas-ready). **Do this BEFORE deploy** (owner ruling 2026-07-17): the first public URL is more shareable once rooms/characters have real art instead of placeholder rectangles.
- **Deploy — PARKED until after the art pass** (owner ruling 2026-07-17). Target when it happens: **Vercel** (chosen for least friction + clean root-domain URLs, which the `?seed=`/`?seed=&challenge=` sharing story wants; repo is already on GitHub so Git-integration auto-deploys). Runner-up if avoiding a new account: GitHub Pages (needs an Actions workflow + Vite `base: '/Hospital-Simms/'` for the project subpath). The build output is a pure static site (`vite build` → `dist/`), portable to any static host — this is a hosting choice, not a dependency.
- **Persistence Phase 2 (seed challenges)** is unblocked — scoped in `docs/PERSISTENCE_PLAN.md`; needs only the Phase-1 determinism guarantees, now in place. Phase 1 owner rulings on record: paused-modal close restores speed 0 (deliberate DailyReportModal deviation), `?load=` persists in the URL like `?seed=`, map dims are baked into saves (comment beside `BALANCE.map`).
