...
- **CHARACTER ART — Option A (procedural polish) SHIPPED 2026-07-20; Tier B/C atlas SCOPED in `docs/CHARACTER_ATLAS_PLAN.md`.** Owner asked how much effort to upgrade the characters, weighed the options (§11 of the plan: commission tint-layer art / buy an asset pack / procedural polish / AI / hybrid), and chose **Option A for now**: an in-house polish of the procedural `drawCharacter` (softer two-layer contact shadow, translucent hair sheen, warmer faces — cheeks + eye catchlights + a subtle smile, a touch more form). Render-only, no save bump, reviewed SHIP. See CHANGELOG *(char polish)*.
  - **Tier B (palette-swap atlas) remains fully scoped + reviewed in the plan** (§1 contract change RATIFIED; both pre-impl reviews folded into v2; the archetype/`LayerSpec` contract, anchor spec, and Option-B staging are ready). Pick it up if/when the owner wants the bigger visible jump and sources art. **Tier C (per-role asset pack) is the alternative** that simplifies the code but needs art for all ~11 roles — see the plan's §11 art-source options + the marketplaces list (itch.io / Kenney / CraftPix / commission).
- **EQUIPMENT ART — CT/MRI SHIPPED procedurally (2026-07-22); X-ray / gamma camera / OR table are the FOLLOW-UP.** Owner asked for scanner sprites; none fit our 2:1-iso flat style (GitHub/marketplaces checked), so `ctGantry`/`mriBore` were redrawn procedurally in `render/sprites.ts` (CT = a compact bore ring + bed; MRI = a long 2-tile tunnel + bed nub — see CHANGELOG *(scanner props)*). The pattern is reusable: `drawPrism` (shared box), `CUSTOM_PROPS` now takes a `slice`, and a 2-tile prop can draw a different half per slice (the MRI's two slices join because west's SE end and east's NW end map to the same screen point). **FOLLOW-UP: give `xrayMachine` (C-arm/emitter), `gammaCamera` (a big detector head), and `orTable` the same custom-silhouette treatment** — they still render as box prisms + a `panel` decor. Watch the per-tile pad limit (x 0..64, y −24..32): a diagonal tube caps at radius ~14 before it spills the tile texture. Also DONE this session: the **hover-tile glow now renders UNDER actors** (it was in the top overlay group above `sortedLayer`; moved below — CHANGELOG *(glow fix)*).
- **Optional art polish (art-review, not defects):** the three green-family scrub-cap colors were SPREAD (2026-07-20 quick win — RT lime / surgeon mid-green / nurse teal, guarded by the hue-separation test). Remaining pair worth a nudge: `evs` (brown ~29°) and `maintenance` (orange ~22°) sit ~7° apart — disambiguated today by lightness + no scrub cap, so low priority. Same render-only, test-guarded pattern as the scrub-cap spread.
