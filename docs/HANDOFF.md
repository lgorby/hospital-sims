...
- **Deploy** (Vercel — the only remaining tech-plan §6 stretch item besides art).
- **Art pass** (tech plan §2.6 contract — `characterKey()`, the prop-slice lookups, and the `PROP_DECOR` table are atlas-ready).
