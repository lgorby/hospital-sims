...
- **CHARACTER SPRITE ATLAS (Tier B) — SCOPED, not built: `docs/CHARACTER_ATLAS_PLAN.md`.** Owner asked (2026-07-20) how much effort it is to upgrade the characters to better sprites/images. The plan: a palette-swap atlas — author a few grayscale garment ARCHETYPES, tint per role from `ROLE_DEFS.color` at boot, bake into the SAME `characterKey` map the renderer already reads (so the actor hot path never changes). **Code ~1–2 days (Stage 1, ships art-ready with no visible change); the visible upgrade is Stage 2 and is gated on sourcing art (the real cost).** DRAFT / NOT READY — needs owner sign-off on the §2.6 "100% procedural art" contract change (it introduces asset files; determinism unaffected, procedural stays as the fallback) + 2 pre-impl reviews before code. Read the plan for the archetype mapping, the anchor/bounds art spec, and the staging.
